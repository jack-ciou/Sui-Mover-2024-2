...
export function isDisplay(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V26}::display::Display` + '<'); }
...
export function isDisplayCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V26}::display::DisplayCreated` + '<'); }
...
export class DisplayCreated<T0 extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V26}::display::DisplayCreated`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = DisplayCreated.$typeName; readonly $fullTypeName: `${typeof PKG_V26}::display::DisplayCreated<${PhantomToTypeStr<T0>}>`; readonly $typeArgs: [PhantomToTypeStr<T0>]; readonly $isPhantom = DisplayCreated.$isPhantom;

 readonly id: ToField<ID>

 private constructor(typeArgs: [PhantomToTypeStr<T0>], fields: DisplayCreatedFields<T0>, ) { this.$fullTypeName = composeSuiType( DisplayCreated.$typeName, ...typeArgs ) as `${typeof PKG_V26}::display::DisplayCreated<${PhantomToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id; }

 static reified<T0 extends PhantomReified<PhantomTypeArgument>>( T0: T0 ): DisplayCreatedReified<ToPhantomTypeArgument<T0>> { return { typeName: DisplayCreated.$typeName, fullTypeName: composeSuiType( DisplayCreated.$typeName, ...[extractType(T0)] ) as `${typeof PKG_V26}::display::DisplayCreated<${PhantomToTypeStr<ToPhantomTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T0>>], isPhantom: DisplayCreated.$isPhantom, reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => DisplayCreated.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => DisplayCreated.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => DisplayCreated.fromBcs( T0, data, ), bcs: DisplayCreated.bcs, fromJSONField: (field: any) => DisplayCreated.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => DisplayCreated.fromJSON( T0, json, ), fromSuiParsedData: (content: SuiParsedData) => DisplayCreated.fromSuiParsedData( T0, content, ), fromSuiObjectData: (content: SuiObjectData) => DisplayCreated.fromSuiObjectData( T0, content, ), fetch: async (client: SuiClient, id: string) => DisplayCreated.fetch( client, T0, id, ), new: ( fields: DisplayCreatedFields<ToPhantomTypeArgument<T0>>, ) => { return new DisplayCreated( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
...
export function isVersionUpdated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V26}::display::VersionUpdated` + '<'); }
