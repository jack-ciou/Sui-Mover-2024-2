...
export function isVersioned(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V26}::versioned::Versioned`; }
...
export class Versioned implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V26}::versioned::Versioned`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Versioned.$typeName; readonly $fullTypeName: `${typeof PKG_V26}::versioned::Versioned`; readonly $typeArgs: []; readonly $isPhantom = Versioned.$isPhantom;

 readonly id: ToField<UID>; readonly version: ToField<"u64">

 private constructor(typeArgs: [], fields: VersionedFields, ) { this.$fullTypeName = composeSuiType( Versioned.$typeName, ...typeArgs ) as `${typeof PKG_V26}::versioned::Versioned`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.version = fields.version; }

 static reified( ): VersionedReified { return { typeName: Versioned.$typeName, fullTypeName: composeSuiType( Versioned.$typeName, ...[] ) as `${typeof PKG_V26}::versioned::Versioned`, typeArgs: [ ] as [], isPhantom: Versioned.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Versioned.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Versioned.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Versioned.fromBcs( data, ), bcs: Versioned.bcs, fromJSONField: (field: any) => Versioned.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Versioned.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Versioned.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => Versioned.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => Versioned.fetch( client, id, ), new: ( fields: VersionedFields, ) => { return new Versioned( [], fields ) }, kind: "StructClassReified", } }
 ...
 static fromSuiObjectData( data: SuiObjectData ): Versioned { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isVersioned(data.bcs.type)) { throw new Error(`object at is not a Versioned object`); }

 return Versioned.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Versioned.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
...
export function isVersionChangeCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V26}::versioned::VersionChangeCap`; }
...
export class VersionChangeCap implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): VersionChangeCap { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isVersionChangeCap(data.bcs.type)) { throw new Error(`object at is not a VersionChangeCap object`); }

 return VersionChangeCap.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return VersionChangeCap.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
