...
export function isReferent(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V26}::borrow::Referent` + '<'); }
...
export class Referent<T0 extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V26}::borrow::Referent`; static readonly $numTypeParams = 1; static readonly $isPhantom = [false,] as const;

 readonly $typeName = Referent.$typeName; readonly $fullTypeName: `${typeof PKG_V26}::borrow::Referent<${ToTypeStr<T0>}>`; readonly $typeArgs: [ToTypeStr<T0>]; readonly $isPhantom = Referent.$isPhantom;

 readonly id: ToField<"address">; readonly value: ToField<Option<T0>>

 private constructor(typeArgs: [ToTypeStr<T0>], fields: ReferentFields<T0>, ) { this.$fullTypeName = composeSuiType( Referent.$typeName, ...typeArgs ) as `${typeof PKG_V26}::borrow::Referent<${ToTypeStr<T0>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.value = fields.value; }

 static reified<T0 extends Reified<TypeArgument, any>>( T0: T0 ): ReferentReified<ToTypeArgument<T0>> { return { typeName: Referent.$typeName, fullTypeName: composeSuiType( Referent.$typeName, ...[extractType(T0)] ) as `${typeof PKG_V26}::borrow::Referent<${ToTypeStr<ToTypeArgument<T0>>}>`, typeArgs: [ extractType(T0) ] as [ToTypeStr<ToTypeArgument<T0>>], isPhantom: Referent.$isPhantom, reifiedTypeArgs: [T0], fromFields: (fields: Record<string, any>) => Referent.fromFields( T0, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Referent.fromFieldsWithTypes( T0, item, ), fromBcs: (data: Uint8Array) => Referent.fromBcs( T0, data, ), bcs: Referent.bcs(toBcs(T0)), fromJSONField: (field: any) => Referent.fromJSONField( T0, field, ), fromJSON: (json: Record<string, any>) => Referent.fromJSON( T0, json, ), fromSuiParsedData: (content: SuiParsedData) => Referent.fromSuiParsedData( T0, content, ), fromSuiObjectData: (content: SuiObjectData) => Referent.fromSuiObjectData( T0, content, ), fetch: async (client: SuiClient, id: string) => Referent.fetch( client, T0, id, ), new: ( fields: ReferentFields<ToTypeArgument<T0>>, ) => { return new Referent( [extractType(T0)], fields ) }, kind: "StructClassReified", } }
 ...
 id: this.id,value: fieldToJSON<Option<T0>>(`${Option.$typeName}<${this.$typeArgs[0]}>`, this.value),
 ...
 static fromJSON<T0 extends Reified<TypeArgument, any>>( typeArg: T0, json: Record<string, any> ): Referent<ToTypeArgument<T0>> { if (json.$typeName !== Referent.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeSuiType(Referent.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )

 return Referent.fromJSONField( typeArg, json, ) }

 static fromSuiParsedData<T0 extends Reified<TypeArgument, any>>( typeArg: T0, content: SuiParsedData ): Referent<ToTypeArgument<T0>> { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isReferent(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a Referent object`); } return Referent.fromFieldsWithTypes( typeArg, content ); }

 static fromSuiObjectData<T0 extends Reified<TypeArgument, any>>( typeArg: T0, data: SuiObjectData ): Referent<ToTypeArgument<T0>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isReferent(data.bcs.type)) { throw new Error(`object at is not a Referent object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressSuiType(gotTypeArgs[0]); const expectedTypeArg = compressSuiType(extractType(typeArg)); if (gotTypeArg !== compressSuiType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return Referent.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return Referent.fromSuiParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T0 extends Reified<TypeArgument, any>>( client: SuiClient, typeArg: T0, id: string ): Promise<Referent<ToTypeArgument<T0>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Referent object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isReferent(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Referent object`); }
...
export function isBorrow(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V26}::borrow::Borrow`; }
